    ...
    <div className="min-h-screen bg-gray-50 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-2 cursor-pointer" onClick={() => navigate('/')}>
              ...
              <h1 className="text-2xl font-bold bg-brand-600 bg-clip-text text-transparent">CreatorX</h1>
            </div>
            <div className="flex items-center space-x-4">
              <button 
                onClick={() => navigate('/login')}
                className="px-5 py-2 text-gray-700 hover:text-brand-600 font-medium transition-colors"
              >
                Sign In
              </button>
              <button 
                onClick={() => navigate('/signup')}
                className="px-6 py-2 bg-brand-600 text-white rounded-full hover:bg-brand-700 transition-all transform hover:scale-105 shadow-lg font-medium"
              >
                Get Started
              </button>
            </div>
      ...
      <section className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        <div className="text-center relative z-10">
          <div className="inline-flex items-center space-x-2 bg-brand-100 text-brand-700 px-4 py-2 rounded-full mb-6 animate-bounce">
          ...
          <div className="flex flex-col sm:flex-row justify-center items-center space-y-4 sm:space-y-0 sm:space-x-6 mb-12">
            <button 
              onClick={() => navigate('/signup')}
              className="group px-8 py-4 bg-brand-600 text-white rounded-full text-lg font-semibold hover:bg-brand-700 transition-all transform hover:scale-105 shadow-2xl flex items-center space-x-2"
            >
      ...
      <section className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          ...
          />
          <FeatureCard
            icon={<Type className="w-8 h-8 text-brand-600" />}
...
function FeatureCard({ icon, title, description }: FeatureCardProps) {
  return (
    <div className="group card hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-2 border-2 border-transparent hover:border-purple-200 cursor-pointer">
      ...
      <h4 className="text-xl font-semibold mb-2 group-hover:text-brand-600 transition-colors">{title}</h4>
